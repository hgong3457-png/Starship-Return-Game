...
void ShowMenu() {
    ...
    while (true) {
        ...
        if (!showIntro) {
            ...
            drawtext(_T("  “空旅号”勘探舰因一次意外，于宇宙中失联漂泊多年。有一天忽然捕捉到地球的微弱脉冲，于是它调整航向全速返航。沿途各类陨石群，小行星带，大型天体和黑洞引力陷阱接踵而至，你将操控这艘星际飞船精准规划路线，并可借助钨合金子弹来击碎障碍，助它成功回家。                                       该游戏通过WASD操控移动，空格为发射子弹的按钮。小型障碍物（随机对应灰色/黄色球体）可以击碎，大型天体（对应蓝/红色）击打无效，且靠近它一定距离范围内会受到万有引力作用导致撞击。黑洞会有更强更广的引力场作用，一旦被吞噬游戏直接结束，返航失败。游戏界面左上方代表返航进程，本游戏通过击碎的小型障碍物数目来量化返航进度，达到20个即视为成功返航。右上方为实时血量（护盾值），共10格。撞击到小型扣除1格，大型扣2格，黑洞则直接清零。"), &textRect, DT_WORDBREAK);
